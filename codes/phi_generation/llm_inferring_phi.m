function S = llm_inferring_phi(user_input, varargin)
% LLM_INFERRING_PHI  LLM priors for phi/PSC experiments (includes phi_expr in JSON).
%   The repository copies in codes/llm_inferring.m match main; this file is used by
%   run_llm_pipeline.m and other scripts in codes/phi_generation/ only.
...
prompt = [
    ...
    " - phi_expr: You are a mathematical control theory expert designing the PSC barrier for this user. " + ...
    llm_phi_barrier_rules_text() + ...
    "   Align phi_expr with the user's driving preference (conservative vs aggressive) when it does not conflict with the constraints." + ...
    " Policy:" + ...
    " - If the user uses vague words (""seems"", ""maybe"", ""not sure"", ""probably""), or does not sound certain, use sigma_0=0.3." + ...
    " - Keep bar_sigma=0.3" + ...
    " - Only change e_max with explicit user cues about aggressiveness or precision." + ...
...
if contains(p.Results.ModelName, "gpt", "IgnoreCase", true)
    ...
    resp = generate(model, user_input, Temperature=0.9, MaxNumTokens=300);
