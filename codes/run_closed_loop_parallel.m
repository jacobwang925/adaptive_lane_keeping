function res = run_closed_loop_parallel(prior_ic, mes_var, emax, mu_gt, v0, saveData)
...
if saveData
    % Format parameters for filename (replace '.' with 'p')
    ...
    v0_str = strrep(num2str(v0), '.', 'p');

    % Create the dynamic parameter string
    if v0 == 20 % default value
        param_str = sprintf('_prior_%s_%s_mesvar_%s_emax_%s', ...
                        p1_str, p2_str, mes_var_str, emax_str);
    else
        param_str = sprintf('_prior_%s_%s_mesvar_%s_emax_%s_v0_%s', ...
                        p1_str, p2_str, mes_var_str, emax_str, v0_str);
    end

    % 1. AMPC filename
    file_ampc = ['data_mpc/data_AMPC_multi_', condition_str, '_H10', param_str, '.mat'];
    save(file_ampc, 'MU', 'PROB', 'SPEED', 'TRAJ', 'STATE', 'ETIME', 'v0', 'V0'); % <-- include v0, V0
    fprintf('AMPC = load("%s");\n', file_ampc);
end
...
if saveData
    ...
    mes_var_str = strrep(num2str(mes_var), '.', 'p');
    emax_str = strrep(num2str(emax), '.', 'p');
    v0_str = strrep(num2str(v0), '.', 'p');  % <-- add v0 info
    ...
    save(file_cdbf, 'MU', 'PROB', 'SPEED', 'TRAJ', 'STATE', 'ETIME', 'v0', 'V0'); % <-- include v0, V0
    fprintf('CDBF = load("%s");\n', file_cdbf);
end
...
if saveData
    ...
    save(file_apsc, 'MU', 'PROB', 'SPEED', 'TRAJ', 'STATE', 'ETIME', 'v0', 'V0'); % <-- include v0, V0
    fprintf('APSC = load("%s");\n', file_apsc);
end
