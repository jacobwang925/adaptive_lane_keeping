...
% load data
safeAdap  = load("data_SafeAdap_mu09.mat");
fixedSafe = load("data_FixedSafe_mu09.mat");
nominal   = load("data_Nominal_mu09.mat");
...
plot(seconds(safeAdap.prob.Time), safeAdap.prob.prob,Color='b', DisplayName='Safe adaptive controller', LineStyle='--')
...
plot(seconds(safeAdap.state.Time),  safeAdap.state.state(:,1)*3600/1000,  Color='b', DisplayName='Safe adaptive controller', LineStyle='--')
...
saveas(f,'fig_mu09_velocity','epsc')
...
plot(seconds(safeAdap.state.Time),  safeAdap.state.state(:,4)*180/pi,  Color='b',  DisplayName='Safe adaptive controller', LineStyle='--')
hold off
xlabel('Time / s')
...
saveas(f,'fig_mu09_steer_angle','epsc')
...
plot(seconds(safeAdap.state.Time),  safeAdap.state.state(:,11),  Color='b', DisplayName='Safe adaptive controller', LineStyle='--')
...
ylim([-15 5])
%legend(Location="best")
saveas(f,'fig_mu09_lat_error','epsc')
