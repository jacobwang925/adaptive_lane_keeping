...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% load data
safeAdap_dry = load("../data/data_SafeAdap_multi_dry.mat");
fixdSafe_dry = load("../data/data_FixedSafe_multi_dry.mat");
nomCtrol_dry = load("../data/data_Nominal_multi_dry.mat");

safeAdap_wet = load("../data/data_SafeAdap_multi_wet.mat");
fixdSafe_wet = load("../data/data_FixedSafe_multi_wet.mat");
nomCtrol_wet = load("../data/data_Nominal_multi_wet.mat");

safeAdap_icy = load("../data/data_SafeAdap_multi_icy.mat");
fixdSafe_icy = load("../data/data_FixedSafe_multi_icy.mat");
nomCtrol_icy = load("../data/data_Nominal_multi_icy.mat");

mpc_psc_icy  = load("data_PCS_multi_icy.mat");
...
P_mpc_pcs_icy   = mean( rmmissing(reshape(mpc_psc_icy.PROB, [],1)) );
...
data = [P_safeAdap_dry, P_fixdSafe_dry, P_nomCtrol_dry, 0;
        P_safeAdap_wet, P_fixdSafe_wet, P_nomCtrol_wet, 0;
        P_safeAdap_icy, P_fixdSafe_icy, P_nomCtrol_icy, P_mpc_pcs_icy];
bar(X, data)
...
V_mpc_pcs_icy  = mean( rmmissing(reshape(mpc_psc_icy.SPEED, [],1)) );
...
X = categorical({'Dry','Wet','Icy'});
X = reordercats(X,{'Dry','Wet','Icy'});
data = [V_safeAdap_dry, V_fixdSafe_dry, V_nomCtrol_dry, 0;
        V_safeAdap_wet, V_fixdSafe_wet, V_nomCtrol_wet, 0;
        V_safeAdap_icy, V_fixdSafe_icy, V_nomCtrol_icy, V_mpc_pcs_icy]*3600./1000 ;
bar(X, data)
pmin.HandleVisibility = 'off';
hold off
xlabel('Road condition')
...
for i = 1:num
    ...
    % mpc    
    traj_x = rmmissing( mpc_psc_icy.TRAJ(i, :, 1) );
    traj_y = rmmissing( mpc_psc_icy.TRAJ(i, :, 2) );
    plot(traj_x, traj_y, Color="#7E2F8E", HandleVisibility='off')
